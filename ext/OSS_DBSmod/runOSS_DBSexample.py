...
run_full_model(master_dict) governs the simulation flow taking master_dict as the input that will modify the input dictionary (defined in HelperOSS.py)
...
sys.path.append("/media/storage/cDBS/ext/OSS_DBS/OSS_platform/")
import numpy as np
import time
import pickle
import subprocess
...
from ext.OSS_DBSmod.HelperOSS import *
from utils.HelperFunctions import FileOperations, Imaging

folder2savedata = os.path.join(ROOTDIR, 'data', 'NIFTI', 'test')
FileOperations.create_folder(folder2savedata)

start_simulation_run = time.time()
master_dict = {}
d = get_default_dict()
d = rearrange_dict_settings(d)
d.update(master_dict)  # Not sure what this is exactly for ?!

## Add the entire part included in check_state

MRI = Imaging.preprocessImaging_OSS(filename=os.path.join(ROOTDIR, 'ext', 'OSS_DBS', 'OSS_platform',
                                                          'icbm_avg_152_segmented.nii.gz'))

if not os.path.isfile(os.path.join(folder2savedata, 'Brain_substitute.brep')):
    print('create_BrainTemplate')
...
def run_full_model(master_dict):
    ...
    d.update(
        master_dict)  # modifies the user provided input dictionary (e.g. for UQ study), check run_master_study() function . Warning: this does not work update the encap. layer properties and the solver during adaptive mesh refiment, because these data are realoaded from the original dictionary
    ...
    check_state(
        d)  # will switch the state of simulation depending on the corresp. data in the input_dictionary, will also manage project folders
    ...
    if d["DTI_data_name"] != 0:  # 0 if we do not provide DTI data
        ...
        DTI_param = obtain_DTI_class(d,
                                     MRI_param)  # also creates 'MRI_DTI_derived_data/Tensor_array_DTI.csv' and meta data
    ...
    if d["Init_mesh_ready"] == 0:
        ...
        Domains = build_final_geometry(d, MRI_param, Brain_shape_name, N_array.ROI_radius,
                                       cc_multicontact)  # creates and discretizes the geometry with the implanted electrode, encapsulation layer and ROI, converts to the approp. format. The files are stored in Meshes/
    ...
    Phi_vector = [x for x in d["Phi_vector"] if
                  x is not None]  # now we don't need None values (floating potentials), Contacts contain only the active ones
    Domains.fi = Phi_vector
    with open('Meshes/Mesh_ind.file', "wb") as f:
        pickle.dump(Domains, f, pickle.HIGHEST_PROTOCOL)

    if d[
        "current_control"] == 1 and cc_multicontact == False:  # if multicontact, then it will be scaled in parallel comp.
    ...
    if d["Skip_mesh_refinement"] == 1:
        ...
        if d["refinement_frequency"][0] == -1:  # frequencies basing on the power spectrum distribution
            from Signal_generator import pick_refinement_freqs
            ref_freqs, Xs_on_ref_freqs = pick_refinement_freqs(FR_vector_signal, Xs_signal_norm, d["num_ref_freqs"],
                                                               A)  # A is needed to switch back to unit pulse. We use unit pulse in adaptive to avoid confusion (math modules always scale the signal to the required value)
        else:
    ...
    with open(os.devnull, 'w') as FNULL:
        subprocess.call('python Visualization_files/Paraview_CSFref.py', shell=True, stdout=FNULL,
    ...
    if d["Adapted_mesh_ready"] == 0:
        ...
        Ampl_on_vert = mesh_adapter(MRI_param, DTI_param, Scaling_CSF, Domains, d, anisotrop, cc_multicontact,
                                    ref_freqs)  # also saves adapted mesh
        np.savetxt('Results_adaptive/Ampl_on_vert.csv', Ampl_on_vert, delimiter=" ")  # just to check

    with open(os.devnull, 'w') as FNULL:
        subprocess.call('python Visualization_files/Paraview_adapted.py', shell=True, stdout=FNULL,
                        stderr=subprocess.STDOUT)
    ...
    if d["Parallel_comp_ready"] == 0:
        ...
        if ["Parallel_comp_interrupted"] == 1:
            import os
            if not (os.path.isfile('Field_solutions/Phi_real_scaled_' + str(d["freq"]) + 'Hz.pvd') or os.path.isfile(
                    'Field_solutions/Phi_real_unscaled_' + str(
        ...
        if d["spectrum_trunc_method"] == 'No Truncation':
            ...
            calculate_in_parallel(d, FR_vector_signal, Domains, MRI_param, DTI_param, anisotrop, number_of_points,
                                  cc_multicontact)
        ...
        if d["spectrum_trunc_method"] == 'High Amplitude Method' or d["spectrum_trunc_method"] == 'Cutoff Method' or d[
            ...
            calculate_in_parallel(d, FR_vector_signal_new, Domains, MRI_param, DTI_param, anisotrop, number_of_points,
                                  cc_multicontact)
    else:
    ...
    if d["spectrum_trunc_method"] == 'No Truncation':
        ...
        name_sorted_solution = 'Field_solutions/sorted_solution_' + str(d["spectrum_trunc_method"]) + '_' + str(
            d["trunc_param"]) + '.csv'
    ...
    if d["IFFT_ready"] == 0 and d["Full_Field_IFFT"] == 1:
        ...
        VTA_size = get_field_in_time(d, FR_vector_signal, Xs_signal_norm, t_vector, N_array.N_models, N_array.pattern[
            'num_segments'])  # also uses data from Field_solutions_functions/ and if spectrum truncation is applied, than data from Stim_Signal/
        ...
        if d["spectrum_trunc_method"] == 'High Amplitude Method' or d["spectrum_trunc_method"] == 'Cutoff Method':
            ...
            if isinstance(d["n_Ranvier"], list):  # if different populations
                ...
                for i in range(len(d["n_Ranvier"])):
                    ...
                    last_point = convolute_signal_with_field_and_compute_ifft(d, Xs_signal_norm, N_array.N_models[i],
                                                                              N_array.pattern['num_segments'][i],
                                                                              FR_vector_signal, t_vector, A,
                                                                              'Field_solutions/sorted_solution.csv',
                                                                              dif_axons=True, last_point=last_point)
            else:
                convolute_signal_with_field_and_compute_ifft(d, Xs_signal_norm, N_array.N_models,
                                                             N_array.pattern['num_segments'], FR_vector_signal,
                                                             t_vector, A, 'Field_solutions/sorted_solution.csv')
        else:
            ...
            if isinstance(d["n_Ranvier"], list):  # if different populations
                ...
                for i in range(len(d["n_Ranvier"])):
                    ...
                    last_point = convolute_signal_with_field_and_compute_ifft(d, Xs_signal_norm_new,
                                                                              N_array.N_models[i],
                                                                              N_array.pattern['num_segments'][i],
                                                                              FR_vector_signal_new, t_vector, A,
                                                                              name_sorted_solution, dif_axons=True,
                                                                              last_point=last_point)
            else:
    ...
    if d["Axon_Model_Type"] == 'McIntyre2002':
        ...
        with open(os.devnull, 'w') as FNULL:
            subprocess.call('nrnivmodl axnode', shell=True, stdout=FNULL, stderr=subprocess.STDOUT)
    ...
    if isinstance(d["n_Ranvier"], list) and len(d["n_Ranvier"]) > 1:
        ...
        last_point = 0
        for i in range(len(d["n_Ranvier"])):
            Number_of_activated_population = run_simulation_with_NEURON(last_point, i, d["diam_fib"][i],
                                                                        1000 * d["t_step"], 1000.0 / d["freq"],
                                                                        d["n_Ranvier"][i], N_array.N_models[i],
                                                                        d["v_init"], t_vector.shape[0], d["Ampl_scale"],
        ...
        Number_of_activated = run_simulation_with_NEURON(0, -1, d["diam_fib"], 1000 * d["t_step"], 1000.0 / d["freq"],
                                                         d["n_Ranvier"], N_array.N_models, d["v_init"],
                                                         t_vector.shape[0], d["Ampl_scale"], d["number_of_processors"])

    if isinstance(d["n_Ranvier"], list) and len(d["n_Ranvier"]) > 1:
        with open(os.devnull, 'w') as FNULL:
            subprocess.call('python Visualization_files/Paraview_connections_activation.py', shell=True, stdout=FNULL,
                            stderr=subprocess.STDOUT)
        if d['Show_paraview_screenshots'] == 1:
...
def run_master_study():
    ...
    for i in range(len(Salome_profile_list)):
        master_dict = {'Electrode_type': Salome_profile_list[i], 'voxel_arr_MRI': 1, 'voxel_arr_DTI': 1,
                       'Init_neuron_model_ready': 1}  # some steps were already done
        Result_diff_profiles[i, :] = run_full_model(master_dict)

    print("Number of activated neurons, the benchmark and the profiles : ", Number_of_activated_bench,
    ...
    print("Activation for Full_run,High_ampl,Octaves: ", Number_of_activated_benchmark, Number_of_activated_high_ampl,
          Number_of_activated_octaves)


# run_master_study()    #in case we want to find optimal spectrum truncation method and initial mesh settings.

master_dict = {}  # you can implement UQ or optimization by adding a function that will create master_dict with entries to be optimized. Name of entries should be the same as in HelperOSS.py
Run_time_high_ampl, Number_of_activated_high_ampl = run_full_model(master_dict)
